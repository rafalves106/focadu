...
public class GetWeeklyDetailUseCase
{
    ...
    public async Task<WeeklyDetailDto> ExecuteAsync(Guid weeklyId, CancellationToken cancellationToken = default)
    {
        ...
        var dailyDtos = weekly.Dailies
            ...
            .Select(d => new DailyOverviewDto(
                ...
                d.Activities.Count,
                d.Activities.Count(a => a.Status == ActivityStatus.Completed),
                d.Activities.Count(a => a.Responses.Any(r => r.Passed))))
            .ToList();

        var contentDtos = weekly.CuratedContents
            .Select(c => new CuratedContentDto(c.Id, c.Type, c.Title, c.ExternalUrl, c.BodyText))
            .ToList();

        var projectDto = weekly.Project is null
            ? null
            : new WeeklyProjectDto(weekly.Project.Id, weekly.Project.SpecText, weekly.Project.Status, weekly.Project.SubmissionUrl);
