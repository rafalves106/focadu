...
const PODIUM = [
  { position: 2, height: 'h-16' },
  { position: 1, height: 'h-[88px]' },
  { position: 3, height: 'h-12' },
];
...
export function SquadRanking({
  ...
  return (
    <section className="flex min-w-0 flex-col gap-3 border-2 border-stroke bg-base p-5 lg:min-h-0 lg:w-[472px] lg:shrink-0">
      <PanelLabel
        aside={
      ...
      <ScrollArea className="lg:min-h-0 lg:flex-1" contentClassName="flex flex-col gap-3 lg:pr-4">
        ...
        {top.length > 0 && (
          <div className="flex justify-center">
            {PODIUM.map(({ position }) => {
              ...
              return (
                <span key={position} className="flex w-[104px] flex-col items-center gap-1">
                  {entry && (
                    <>
                      <span className={`max-w-full truncate font-pixel-label text-[9px] ${you ? 'text-accent' : 'text-primary'}`}>{you ? 'Você' : entry.displayName}</span>
        ...
        <ol className="flex flex-col gap-1.5">
          {(isOwner ? [...top, ...rest] : rest).map((entry) => {
            const you = entry.userId === userId;
            const coLeader = entry.userId === coLeaderUserId;
            const canManage = isOwner && !you;
            return (
              <li key={entry.userId} className="relative">
                <div className={`flex items-center gap-3 border-2 bg-surface px-3 py-1 ${menuFor === entry.userId ? 'border-project' : you ? 'border-accent/60' : 'border-transparent'}`}>
                  ...
                  {canManage && (
                    <button
                      type="button"
                      onClick={() => setMenuFor((v) => (v === entry.userId ? null : entry.userId))}
                      aria-label={`Gerenciar ${entry.displayName}`}
                      aria-expanded={menuFor === entry.userId}
                      className="px-1 font-pixel text-2xl leading-none text-project hover:brightness-125"
                    >
                      ⋯
                    </button>
                  )}
                </div>
                {menuFor === entry.userId && (
                  <div className="absolute top-full right-0 z-10 mt-1 flex w-56 flex-col gap-3 border-2 border-project bg-base p-4 shadow-[6px_6px_0_0_#000]">
...
function PodiumStep({ entry, height, look, you, leader }: { entry: RankingEntryDto; height: string; look: AgentLook | null; you: boolean; leader: boolean }) {
  return (
    <div className="flex w-[104px] flex-col items-center">
      ...
      {look ? <AgentSprite look={look} frame={{ mini: 'baixo', step: 0 }} scale={3} /> : <span className="size-12" />}
      <div className={`flex w-full flex-col items-center border-2 border-b-0 bg-stroke/50 pt-1 ${height} ${you ? 'border-accent' : 'border-secondary/60'}`}>
        ...
        <img src={MEDALS[entry.position - 1]} alt={`${entry.position}º`} className="mt-2 size-8 pixelated" />
      </div>
    </div>
  );
}
